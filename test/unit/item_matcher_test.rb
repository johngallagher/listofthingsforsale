...
class ItemMatcherTest < ActiveSupport::TestCase
  ...
  test "item with same name and price, different description should match" do
    wallet = Factory.build(:wallet_item)
    found_item = ItemMatcher.new(:items => [wallet], :criteria => {:name => "wallet", :description_text => "this is a wallet", :price => 22.22}).match
    assert_equal(wallet, found_item)
  end
end
